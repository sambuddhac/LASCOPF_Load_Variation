...
#include <cmath>
using namespace std;
double GensolverLast::eval_gap(void) {
  int i;
  double gap;
  gap = 0;
  for (i = 0; i < 6; i++)
...
void GensolverLast::set_defaults(void) {
...
void GensolverLast::setup_pointers(void) {
  work.y = work.x + 13;
  work.s = work.x + 13;
  work.z = work.x + 19;
  vars.Pg = work.x + 0;
...
void GensolverLast::setup_indexing(void) {
  setup_pointers();
}
void GensolverLast::set_start(void) {
  int i;
  for (i = 0; i < 13; i++)
    work.x[i] = 0;
  for (i = 0; i < 0; i++)
...
double GensolverLast::eval_objv(void) {
  int i;
  double objv;
  ...
  objv += params.c0[0]+work.quad_399913824256[0]+work.quad_608032620544[0]+work.frac_121674190848*(work.quad_324888432640[0]+work.quad_331507691520[0]);
  return objv;
}
void GensolverLast::fillrhs_aff(void) {
  int i;
  double *r1, *r2, *r3, *r4;
  r1 = work.rhs;
  r2 = work.rhs + 13;
...
void GensolverLast::fillrhs_cc(void) {
  int i;
  double *r2;
...
void GensolverLast::refine(double *target, double *var) {
  int i, j;
...
double GensolverLast::calc_ineq_resid_squared(void) {
...
double GensolverLast::calc_eq_resid_squared(void) {
...
void GensolverLast::better_start(void) {
...
void GensolverLast::fillrhs_start(void) {
...
void GensolverLast::tic(void) {
  tic_timestart = clock();
}
float GensolverLast::toc(void) {
...
float GensolverLast::tocq(void) {
  clock_t tic_timestop;
  tic_timestop = clock();
  return (float)(tic_timestop - tic_timestart) / CLOCKS_PER_SEC;
}
void GensolverLast::printmatrix(char *name, double *A, int m, int n, int sparse) {
  int i, j;
  printf("%s = [...\n", name);
  for (i = 0; i < m; i++) {
    for (j = 0; j < n; j++)
...
double GensolverLast::unif(double lower, double upper) {
...
#define EPS 1.2e-7
#define RNMX (1.0-EPS)
float GensolverLast::ran1(long*idum, int reset) {
  int j;
  long k;
  static long iy=0;
  static long iv[NTAB];
...
/* Next function is from numerical recipes in C. */
float GensolverLast::randn_internal(long *idum, int reset) {
  static int iset=0;
  static float gset;
  float fac, rsq, v1, v2;
  if (reset) {
...
double GensolverLast::randn(void) {
  return randn_internal(&global_seed, 0);
}
void GensolverLast::reset_rand(void) {
  srand(15);
  global_seed = 1;
...
void GensolverLast::multbymA(double *lhs, double *rhs) {
}
void GensolverLast::multbymAT(double *lhs, double *rhs) {
  lhs[0] = 0;
  lhs[1] = 0;
  lhs[2] = 0;
  lhs[3] = 0;
...
void GensolverLast::multbymG(double *lhs, double *rhs) {
  lhs[0] = -rhs[0]*(-1);
  lhs[1] = -rhs[0]*(1);
  lhs[2] = -rhs[0]*(1);
  lhs[3] = -rhs[0]*(-1);
...
void GensolverLast::multbymGT(double *lhs, double *rhs) {
...
void GensolverLast::multbyP(double *lhs, double *rhs) {
...
void GensolverLast::fillq(void) {
  work.q[0] = params.c1[0]-2*params.PgNu[0]*work.frac_479733190656+params.gamma[0]*params.B[0]-params.lambda_4[0]+2*work.frac_121674190848*((-params.Pg_N_init[0]+params.Pg_N_avg[0]+params.ug_N[0])*params.ones[0]+(-params.Pg_N_init[1]+params.Pg_N_avg[1]+params.ug_N[1])*params.ones[1]+(-params.Pg_N_init[2]+params.Pg_N_avg[2]+params.ug_N[2])*params.ones[2]+(-params.Pg_N_init[3]+params.Pg_N_avg[3]+params.ug_N[3])*params.ones[3]+(-params.Pg_N_init[4]+params.Pg_N_avg[4]+params.ug_N[4])*params.ones[4]+(-params.Pg_N_init[5]+params.Pg_N_avg[5]+params.ug_N[5])*params.ones[5]+(-params.Pg_N_init[6]+params.Pg_N_avg[6]+params.ug_N[6])*params.ones[6]+(-params.Pg_N_init[7]+params.Pg_N_avg[7]+params.ug_N[7])*params.ones[7]+(-params.Pg_N_init[8]+params.Pg_N_avg[8]+params.ug_N[8])*params.ones[8]+(-params.Pg_N_init[9]+params.Pg_N_avg[9]+params.ug_N[9])*params.ones[9]+(-params.Pg_N_init[10]+params.Pg_N_avg[10]+params.ug_N[10])*params.ones[10]);
...
void GensolverLast::fillh(void) {
  work.h[0] = -params.PgMin[0];
  work.h[1] = params.PgMax[0];
  work.h[2] = -(params.RgMin[0]-params.PgNext[0]);
  work.h[3] = -(params.PgNext[0]-params.RgMax[0]);
  work.h[4] = -params.RgMin[0];
  work.h[5] = params.RgMax[0];
}
void GensolverLast::fillb(void) {
}
void GensolverLast::pre_ops(void) {
...
void GensolverLast::ldl_solve(double *target, double *var) {
  int i;
  ...
  work.v[19] = target[21]-work.L[2]*work.v[13];
  work.v[20] = target[22]-work.L[3]*work.v[14];
  work.v[21] = target[0]-work.L[4]*work.v[17]-work.L[5]*work.v[18]-work.L[6]*work.v[19]-work.L[7]*work.v[20];
  work.v[22] = target[1];
  work.v[23] = target[23]-work.L[8]*work.v[15]-work.L[9]*work.v[21]-work.L[10]*work.v[22];
  work.v[24] = target[24]-work.L[11]*work.v[16]-work.L[12]*work.v[21]-work.L[13]*work.v[22]-work.L[14]*work.v[23];
  ...
  work.v[16] -= work.L[11]*work.v[24];
  work.v[15] -= work.L[8]*work.v[23];
  work.v[14] -= work.L[3]*work.v[20];
  work.v[13] -= work.L[2]*work.v[19];
...
void GensolverLast::ldl_factor(void) {
  work.d[0] = work.KKT[0];
  if (work.d[0] < 0)
  ...
  work.L[3] = (work.KKT[18])*work.d_inv[14];
  ...
  work.L[8] = (work.KKT[20])*work.d_inv[15];
  ...
  work.L[11] = (work.KKT[22])*work.d_inv[16];
  ...
  work.v[19] = work.KKT[27]-work.L[2]*work.v[13];
  work.d[19] = work.v[19];
  if (work.d[19] > 0)
  ...
  work.v[20] = work.KKT[29]-work.L[3]*work.v[14];
  work.d[20] = work.v[20];
  if (work.d[20] > 0)
  ...
  work.v[15] = work.L[8]*work.d[15];
  work.v[21] = work.L[9]*work.d[21];
  work.v[22] = work.L[10]*work.d[22];
  work.v[23] = work.KKT[37]-work.L[8]*work.v[15]-work.L[9]*work.v[21]-work.L[10]*work.v[22];
  work.d[23] = work.v[23];
  if (work.d[23] > 0)
  ...
  work.v[24] = work.KKT[38]-work.L[11]*work.v[16]-work.L[12]*work.v[21]-work.L[13]*work.v[22]-work.L[14]*work.v[23];
  work.d[24] = work.v[24];
  if (work.d[24] > 0)
...
double GensolverLast::check_factorization(void) {
  ...
  residual += temp*temp;
  temp = work.KKT[18]-work.L[3]*work.d[14]*1;
  residual += temp*temp;
  temp = work.KKT[20]-work.L[8]*work.d[15]*1;
  residual += temp*temp;
  temp = work.KKT[22]-work.L[11]*work.d[16]*1;
  residual += temp*temp;
  temp = work.KKT[23]-work.L[0]*work.d[11]*work.L[0]-1*work.d[17]*1;
  residual += temp*temp;
  temp = work.KKT[25]-work.L[1]*work.d[12]*work.L[1]-1*work.d[18]*1;
  residual += temp*temp;
  temp = work.KKT[27]-work.L[2]*work.d[13]*work.L[2]-1*work.d[19]*1;
  residual += temp*temp;
  temp = work.KKT[29]-work.L[3]*work.d[14]*work.L[3]-1*work.d[20]*1;
  residual += temp*temp;
  temp = work.KKT[37]-work.L[8]*work.d[15]*work.L[8]-1*work.d[23]*1-work.L[9]*work.d[21]*work.L[9]-work.L[10]*work.d[22]*work.L[10];
  residual += temp*temp;
  temp = work.KKT[38]-work.L[11]*work.d[16]*work.L[11]-1*work.d[24]*1-work.L[12]*work.d[21]*work.L[12]-work.L[13]*work.d[22]*work.L[13]-work.L[14]*work.d[23]*work.L[14];
  residual += temp*temp;
...
void GensolverLast::matrix_multiply(double *result, double *source) {
  /* Finds result = A*source. */
  result[0] = work.KKT[31]*source[0]+work.KKT[24]*source[19]+work.KKT[26]*source[20]+work.KKT[28]*source[21]+work.KKT[30]*source[22]+work.KKT[32]*source[23]+work.KKT[33]*source[24];
  result[1] = work.KKT[34]*source[1]+work.KKT[35]*source[23]+work.KKT[36]*source[24];
  ...
  result[21] = work.KKT[16]*source[15]+work.KKT[27]*source[21]+work.KKT[28]*source[0];
  result[22] = work.KKT[18]*source[16]+work.KKT[29]*source[22]+work.KKT[30]*source[0];
  result[23] = work.KKT[20]*source[17]+work.KKT[37]*source[23]+work.KKT[32]*source[0]+work.KKT[35]*source[1];
  result[24] = work.KKT[22]*source[18]+work.KKT[38]*source[24]+work.KKT[33]*source[0]+work.KKT[36]*source[1];
}
double GensolverLast::check_residual(double *target, double *multiplicand) {
...
void GensolverLast::fill_KKT(void) {
...
//Workspace work;
//Settings settings;
GensolverLast::GensolverLast( int numberOfCases, double cost2, double cost1, double cost0, double PgenMax, double PgenMin, double RgenMax, double RgenMin, double Beta, double Gamma ) {
...
GensolverLast::~GensolverLast() {
}
#define NUMTESTS 0
int GensolverLast::mainsolve(int APPItCount, double gsRho, double Pgprev[], double Pgavrg[], double Pprice[], double Apriceavg[], double Aavg[], double Aprice[], double PgenPrevAPP, double PgenAPP, double AAPP, double BAPP, double LambAPP3, double LambAPP4 {
  int num_iters;
#if (NUMTESTS > 0)
  int i;
  double time;
  ...
  set_defaults();
  setup_indexing();
  load_default_data(APPItCount, gsRho, Pgprev, Pgavrg, Pprice, Apriceavg, Aavg, Aprice, PgenPrevAPP, PgenAPP, AAPP, BAPP, LambAPP3, LambAPP4);
...
void GensolverLast::load_default_data(int APPItCount, double gsRho, double Pgenprev[], double Pgenavg[], double Powerprice[], double Angpriceavg[], double Angavg[], double Angprice[], double PgenPrevAPP, double PgenAPP, double AAPP, double BAPP, double LambAPP3, double LambAPP4) {
if (APPItCount==1) {
  params.rho[0] = gsRho;
  params.PgPrevNu[0] = 0.0;
  params.PgNu[0] = 0.0;
...
double GensolverLast::getPSol(void) {
 Piterate = (vars.Pg);
 return *Piterate;
}

double GensolverLast::getPPrevSol(void) {
 Piterate = (vars.PgPrev);
 return *PgPreviterate;
}

double GensolverLast::getObj(void) {
 return (params.c2[0])*(*Piterate)*(*Piterate)+(params.c1[0])*(*Piterate)+(params.c0[0]);
}

double *GensolverLast::getThetaPtr(void){
Thiterate = (vars.Thetag);
return Thiterate;
}

double GensolverLast::getRMax() // start of getPMax function
{
	return params.RgMax[0];
} // end of getMax

double GensolverLast::getRMin() // start of getPMin function
{
	return params.RgMin[0];
} // end of getPMin

double GensolverLast::getPMax() // start of getPMax function
{
	return params.PgMax[0];
} // end of getMax

double GensolverLast::getPMin() // start of getPMin function
{
	return params.PgMin[0];
} // end of getPMin

double GensolverLast::getQuadCoeff() // start of getQuadCoeff function
{
	return params.c2[0]; 
} // end of getQuadCoeff

double GensolverLast::getLinCoeff() // start of getLinCoeff function
{
	return params.c1[0]; 
} // end of getLinCoeff

double GensolverLast::getConstCoeff() // start of getConstCoeff function
{
	return params.c0[0]; 
