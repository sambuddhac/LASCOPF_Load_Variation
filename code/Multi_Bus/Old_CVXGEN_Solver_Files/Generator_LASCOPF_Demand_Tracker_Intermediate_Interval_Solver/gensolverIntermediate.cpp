...
#include <cmath>
using namespace std;
double GensolverInter::eval_gap(void) {
  int i;
  double gap;
  gap = 0;
  for (i = 0; i < 6; i++)
...
void GensolverInter::set_defaults(void) {
...
void GensolverInter::setup_pointers(void) {
  work.y = work.x + 14;
  work.s = work.x + 14;
  work.z = work.x + 20;
  vars.Pg = work.x + 0;
  vars.PgNext = work.x + 1;
  vars.PgPrev = work.x + 2;
  vars.Thetag = work.x + 3;
}
void GensolverInter::setup_indexing(void) {
  setup_pointers();
}
void GensolverInter::set_start(void) {
  int i;
  for (i = 0; i < 14; i++)
    work.x[i] = 0;
  for (i = 0; i < 0; i++)
...
double GensolverInter::eval_objv(void) {
  int i;
  double objv;
  ...
  objv += params.c0[0]+work.quad_399913824256[0]+work.quad_608032620544[0]+work.quad_643747205120[0]+work.frac_121674190848*(work.quad_324888432640[0]+work.quad_331507691520[0]);
  return objv;
}
void GensolverInter::fillrhs_aff(void) {
  int i;
  double *r1, *r2, *r3, *r4;
  r1 = work.rhs;
  r2 = work.rhs + 14;
...
void GensolverInter::fillrhs_cc(void) {
  int i;
  double *r2;
...
void GensolverInter::refine(double *target, double *var) {
  int i, j;
...
double GensolverInter::calc_ineq_resid_squared(void) {
...
double GensolverInter::calc_eq_resid_squared(void) {
...
void GensolverInter::better_start(void) {
...
void GensolverInter::fillrhs_start(void) {
...
long GensolverInter::solve(void) {
  int i;
  int iter;
  double *dx, *ds, *dy, *dz;
  double minval;
...
void GensolverInter::tic(void) {
  tic_timestart = clock();
}
float GensolverInter::toc(void) {
...
float GensolverInter::tocq(void) {
  clock_t tic_timestop;
  tic_timestop = clock();
  return (float)(tic_timestop - tic_timestart) / CLOCKS_PER_SEC;
}
void GensolverInter::printmatrix(char *name, double *A, int m, int n, int sparse) {
  int i, j;
  printf("%s = [...\n", name);
  for (i = 0; i < m; i++) {
    for (j = 0; j < n; j++)
...
double GensolverInter::unif(double lower, double upper) {
...
#define EPS 1.2e-7
#define RNMX (1.0-EPS)
float GensolverInter::ran1(long*idum, int reset) {
  int j;
  long k;
  static long iy=0;
  static long iv[NTAB];
...
/* Next function is from numerical recipes in C. */
float GensolverInter::randn_internal(long *idum, int reset) {
  static int iset=0;
  static float gset;
  float fac, rsq, v1, v2;
  if (reset) {
...
double GensolverInter::randn(void) {
  return randn_internal(&global_seed, 0);
}
void GensolverInter::reset_rand(void) {
  srand(15);
  global_seed = 1;
...
void GensolverInter::multbymA(double *lhs, double *rhs) {
}
void GensolverInter::multbymAT(double *lhs, double *rhs) {
  lhs[0] = 0;
  lhs[1] = 0;
  lhs[2] = 0;
  lhs[3] = 0;
...
void GensolverInter::multbymG(double *lhs, double *rhs) {
  lhs[0] = -rhs[0]*(-1);
  lhs[1] = -rhs[0]*(1);
...
void GensolverInter::multbymGT(double *lhs, double *rhs) {
...
void GensolverInter::multbyP(double *lhs, double *rhs) {
...
void GensolverInter::fillq(void) {
...
void GensolverInter::fillh(void) {
...
void GensolverInter::fillb(void) {
}
void GensolverInter::pre_ops(void) {
...
void GensolverInter::ldl_solve(double *target, double *var) {
  int i;
  ...
  work.v[15] = target[18];
  work.v[16] = target[19];
  work.v[17] = target[20]-work.L[0]*work.v[11];
  work.v[18] = target[21]-work.L[1]*work.v[12];
  work.v[19] = target[1];
  work.v[20] = target[22]-work.L[2]*work.v[13]-work.L[3]*work.v[19];
  work.v[21] = target[23]-work.L[4]*work.v[14]-work.L[5]*work.v[19]-work.L[6]*work.v[20];
  work.v[22] = target[0]-work.L[7]*work.v[17]-work.L[8]*work.v[18]-work.L[9]*work.v[20]-work.L[10]*work.v[21];
  work.v[23] = target[2];
  work.v[24] = target[24]-work.L[11]*work.v[15]-work.L[12]*work.v[22]-work.L[13]*work.v[23];
  work.v[25] = target[25]-work.L[14]*work.v[16]-work.L[15]*work.v[22]-work.L[16]*work.v[23]-work.L[17]*work.v[24];
  /* Diagonal scaling. Assume correctness of work.d_inv. */
  for (i = 0; i < 26; i++)
    work.v[i] *= work.d_inv[i];
  /* Back substitution */
  work.v[24] -= work.L[17]*work.v[25];
  work.v[23] -= work.L[13]*work.v[24]+work.L[16]*work.v[25];
  work.v[22] -= work.L[12]*work.v[24]+work.L[15]*work.v[25];
  work.v[21] -= work.L[10]*work.v[22];
  work.v[20] -= work.L[6]*work.v[21]+work.L[9]*work.v[22];
  work.v[19] -= work.L[3]*work.v[20]+work.L[5]*work.v[21];
  work.v[18] -= work.L[8]*work.v[22];
  work.v[17] -= work.L[7]*work.v[22];
  work.v[16] -= work.L[14]*work.v[25];
  work.v[15] -= work.L[11]*work.v[24];
  work.v[14] -= work.L[4]*work.v[21];
  work.v[13] -= work.L[2]*work.v[20];
...
void GensolverInter::ldl_factor(void) {
  work.d[0] = work.KKT[0];
  if (work.d[0] < 0)
  ...
  work.L[2] = (work.KKT[16])*work.d_inv[13];
  ...
  work.L[4] = (work.KKT[18])*work.d_inv[14];
  ...
  work.L[14] = (work.KKT[22])*work.d_inv[16];
  ...
  work.L[7] = (work.KKT[24])*work.d_inv[17];
  ...
  work.L[8] = (work.KKT[26])*work.d_inv[18];
  ...
  work.L[3] = (work.KKT[28])*work.d_inv[19];
  work.L[5] = (work.KKT[29])*work.d_inv[19];
  work.v[13] = work.L[2]*work.d[13];
  work.v[19] = work.L[3]*work.d[19];
  work.v[20] = work.KKT[30]-work.L[2]*work.v[13]-work.L[3]*work.v[19];
  work.d[20] = work.v[20];
  if (work.d[20] > 0)
  ...
  work.L[6] = (-work.L[5]*work.v[19])*work.d_inv[20];
  work.L[9] = (work.KKT[31])*work.d_inv[20];
  work.v[14] = work.L[4]*work.d[14];
  work.v[19] = work.L[5]*work.d[19];
  work.v[20] = work.L[6]*work.d[20];
  work.v[21] = work.KKT[32]-work.L[4]*work.v[14]-work.L[5]*work.v[19]-work.L[6]*work.v[20];
  work.d[21] = work.v[21];
  if (work.d[21] > 0)
  ...
  work.L[10] = (work.KKT[33]-work.L[9]*work.v[20])*work.d_inv[21];
  work.v[17] = work.L[7]*work.d[17];
  work.v[18] = work.L[8]*work.d[18];
  work.v[20] = work.L[9]*work.d[20];
  work.v[21] = work.L[10]*work.d[21];
  work.v[22] = work.KKT[34]-work.L[7]*work.v[17]-work.L[8]*work.v[18]-work.L[9]*work.v[20]-work.L[10]*work.v[21];
  work.d[22] = work.v[22];
  if (work.d[22] < 0)
  ...
  work.L[12] = (work.KKT[35])*work.d_inv[22];
  work.L[15] = (work.KKT[36])*work.d_inv[22];
  work.v[23] = work.KKT[37];
  work.d[23] = work.v[23];
  if (work.d[23] < 0)
  ...
  work.L[13] = (work.KKT[38])*work.d_inv[23];
  work.L[16] = (work.KKT[39])*work.d_inv[23];
  work.v[15] = work.L[11]*work.d[15];
  work.v[22] = work.L[12]*work.d[22];
  work.v[23] = work.L[13]*work.d[23];
  work.v[24] = work.KKT[40]-work.L[11]*work.v[15]-work.L[12]*work.v[22]-work.L[13]*work.v[23];
  work.d[24] = work.v[24];
  if (work.d[24] > 0)
  ...
  work.L[17] = (-work.L[15]*work.v[22]-work.L[16]*work.v[23])*work.d_inv[24];
  work.v[16] = work.L[14]*work.d[16];
  work.v[22] = work.L[15]*work.d[22];
  work.v[23] = work.L[16]*work.d[23];
  work.v[24] = work.L[17]*work.d[24];
  work.v[25] = work.KKT[41]-work.L[14]*work.v[16]-work.L[15]*work.v[22]-work.L[16]*work.v[23]-work.L[17]*work.v[24];
  work.d[25] = work.v[25];
  if (work.d[25] > 0)
...
double GensolverInter::check_factorization(void) {
  /* Returns the squared Frobenius norm of A - L*D*L'. */
  double temp, residual;
  /* Only check the lower triangle. */
  residual = 0;
  temp = work.KKT[34]-1*work.d[22]*1-work.L[7]*work.d[17]*work.L[7]-work.L[8]*work.d[18]*work.L[8]-work.L[9]*work.d[20]*work.L[9]-work.L[10]*work.d[21]*work.L[10];
  residual += temp*temp;
  temp = work.KKT[27]-1*work.d[19]*1;
  residual += temp*temp;
  temp = work.KKT[37]-1*work.d[23]*1;
  residual += temp*temp;
  ...
  residual += temp*temp;
  temp = work.KKT[16]-work.L[2]*work.d[13]*1;
  residual += temp*temp;
  temp = work.KKT[18]-work.L[4]*work.d[14]*1;
  residual += temp*temp;
  temp = work.KKT[20]-work.L[11]*work.d[15]*1;
  residual += temp*temp;
  temp = work.KKT[22]-work.L[14]*work.d[16]*1;
  residual += temp*temp;
  temp = work.KKT[23]-work.L[0]*work.d[11]*work.L[0]-1*work.d[17]*1;
  residual += temp*temp;
  temp = work.KKT[25]-work.L[1]*work.d[12]*work.L[1]-1*work.d[18]*1;
  residual += temp*temp;
  temp = work.KKT[30]-work.L[2]*work.d[13]*work.L[2]-1*work.d[20]*1-work.L[3]*work.d[19]*work.L[3];
  residual += temp*temp;
  temp = work.KKT[32]-work.L[4]*work.d[14]*work.L[4]-1*work.d[21]*1-work.L[5]*work.d[19]*work.L[5]-work.L[6]*work.d[20]*work.L[6];
  residual += temp*temp;
  temp = work.KKT[40]-work.L[11]*work.d[15]*work.L[11]-1*work.d[24]*1-work.L[12]*work.d[22]*work.L[12]-work.L[13]*work.d[23]*work.L[13];
  residual += temp*temp;
  temp = work.KKT[41]-work.L[14]*work.d[16]*work.L[14]-1*work.d[25]*1-work.L[15]*work.d[22]*work.L[15]-work.L[16]*work.d[23]*work.L[16]-work.L[17]*work.d[24]*work.L[17];
  residual += temp*temp;
  temp = work.KKT[24]-1*work.d[17]*work.L[7];
  residual += temp*temp;
  temp = work.KKT[26]-1*work.d[18]*work.L[8];
  residual += temp*temp;
  temp = work.KKT[31]-1*work.d[20]*work.L[9];
  residual += temp*temp;
  temp = work.KKT[28]-work.L[3]*work.d[19]*1;
  residual += temp*temp;
  temp = work.KKT[33]-1*work.d[21]*work.L[10]-work.L[6]*work.d[20]*work.L[9];
  residual += temp*temp;
  temp = work.KKT[29]-work.L[5]*work.d[19]*1;
  residual += temp*temp;
  temp = work.KKT[35]-work.L[12]*work.d[22]*1;
  residual += temp*temp;
  temp = work.KKT[38]-work.L[13]*work.d[23]*1;
  residual += temp*temp;
  temp = work.KKT[36]-work.L[15]*work.d[22]*1;
  residual += temp*temp;
  temp = work.KKT[39]-work.L[16]*work.d[23]*1;
  residual += temp*temp;
  return residual;
}
void GensolverInter::matrix_multiply(double *result, double *source) {
  /* Finds result = A*source. */
  result[0] = work.KKT[34]*source[0]+work.KKT[24]*source[20]+work.KKT[26]*source[21]+work.KKT[31]*source[22]+work.KKT[33]*source[23]+work.KKT[35]*source[24]+work.KKT[36]*source[25];
  result[1] = work.KKT[27]*source[1]+work.KKT[28]*source[22]+work.KKT[29]*source[23];
  result[2] = work.KKT[37]*source[2]+work.KKT[38]*source[24]+work.KKT[39]*source[25];
  result[3] = work.KKT[0]*source[3];
  result[4] = work.KKT[1]*source[4];
  result[5] = work.KKT[2]*source[5];
  result[6] = work.KKT[3]*source[6];
  result[7] = work.KKT[4]*source[7];
  result[8] = work.KKT[5]*source[8];
  result[9] = work.KKT[6]*source[9];
  result[10] = work.KKT[7]*source[10];
  result[11] = work.KKT[8]*source[11];
  result[12] = work.KKT[9]*source[12];
  result[13] = work.KKT[10]*source[13];
  result[14] = work.KKT[11]*source[14]+work.KKT[12]*source[20];
  result[15] = work.KKT[13]*source[15]+work.KKT[14]*source[21];
  result[16] = work.KKT[15]*source[16]+work.KKT[16]*source[22];
  result[17] = work.KKT[17]*source[17]+work.KKT[18]*source[23];
  result[18] = work.KKT[19]*source[18]+work.KKT[20]*source[24];
  result[19] = work.KKT[21]*source[19]+work.KKT[22]*source[25];
  result[20] = work.KKT[12]*source[14]+work.KKT[23]*source[20]+work.KKT[24]*source[0];
  result[21] = work.KKT[14]*source[15]+work.KKT[25]*source[21]+work.KKT[26]*source[0];
  result[22] = work.KKT[16]*source[16]+work.KKT[30]*source[22]+work.KKT[31]*source[0]+work.KKT[28]*source[1];
  result[23] = work.KKT[18]*source[17]+work.KKT[32]*source[23]+work.KKT[33]*source[0]+work.KKT[29]*source[1];
  result[24] = work.KKT[20]*source[18]+work.KKT[40]*source[24]+work.KKT[35]*source[0]+work.KKT[38]*source[2];
  result[25] = work.KKT[22]*source[19]+work.KKT[41]*source[25]+work.KKT[36]*source[0]+work.KKT[39]*source[2];
}
double GensolverInter::check_residual(double *target, double *multiplicand) {
...
void GensolverInter::fill_KKT(void) {
  work.KKT[34] = 2*(params.c2[0]+work.frac_479733190656+work.frac_121674190848*work.quad_877793640448[0]);
  work.KKT[27] = 2*work.frac_479733190656;
  work.KKT[37] = 2*work.frac_479733190656;
...
//Workspace work;
//Settings settings;
GensolverInter::GensolverInter( int numberOfCases, double cost2, double cost1, double cost0, double PgenMax, double PgenMin, double RgenMax, double RgenMin, double Beta, double Gamma ) {
...
GensolverInter::~GensolverInter() {
}
#define NUMTESTS 0
void GensolverInter::mainsolve(int APPItCount, double gsRho, double Pgprev[], double Pgavrg[], double Pprice[], double Apriceavg[], double Aavg[], double Aprice[], double PgenPrevAPP, double PgenAPP, double PgenNextAPP, double AAPP, double BAPP, double DAPP, double LambAPP1, double LambAPP2, double LambAPP3, double LambAPP4) {
  int num_iters;
#if (NUMTESTS > 0)
  int i;
  double time;
  ...
  set_defaults();
  setup_indexing();
  load_default_data(APPItCount, gsRho, Pgprev, Pgavrg, Pprice, Apriceavg, Aavg, Aprice, PgenPrevAPP, PgenAPP, PgenNextAPP, AAPP, BAPP, DAPP, LambAPP1, LambAPP2, LambAPP3, LambAPP4);
...
void GensolverInter::load_default_data(int APPItCount, double gsRho, double Pgenprev[], double Pgenavg[], double Powerprice[], double Angpriceavg[], double Angavg[], double Angprice[], double PgenPrevAPP, double PgenAPP, double PgenNextAPP, double AAPP, double BAPP, double DAPP, double LambAPP1, double LambAPP2, double LambAPP3, double LambAPP4) {
if (APPItCount==1) {
  params.rho[0] = gsRho;
  params.PgPrevNu[0] = 0.0;
  params.PgNu[0] = 0.0;
...
double GensolverInter::getPSol(void) {
 Piterate = (vars.Pg);
 return *Piterate;
}

double GensolverInter::getPPrevSol(void) {
 Piterate = (vars.PgPrev);
 return *PgPreviterate;
}

double GensolverInter::getPNextSol(void) {
 Piterate = (vars.PgNext);
 return *PgNextiterate;
}

double GensolverInter::getObj(void) {
 return (params.c2[0])*(*Piterate)*(*Piterate)+(params.c1[0])*(*Piterate)+(params.c0[0]);
}

double *GensolverInter::getThetaPtr(void){
Thiterate = (vars.Thetag);
return Thiterate;
}

double GensolverInter::getRMax() // start of getPMax function
{
	return params.RgMax[0];
} // end of getMax

double GensolverInter::getRMin() // start of getPMin function
{
	return params.RgMin[0];
} // end of getPMin

double GensolverInter::getPMax() // start of getPMax function
{
	return params.PgMax[0];
} // end of getMax

double GensolverInter::getPMin() // start of getPMin function
{
	return params.PgMin[0];
} // end of getPMin

double GensolverInter::getQuadCoeff() // start of getQuadCoeff function
{
	return params.c2[0]; 
} // end of getQuadCoeff

double GensolverInter::getLinCoeff() // start of getLinCoeff function
{
	return params.c1[0]; 
} // end of getLinCoeff

double GensolverInter::getConstCoeff() // start of getConstCoeff function
{
	return params.c0[0]; 
