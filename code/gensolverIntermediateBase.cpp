...
#include "gensolverIntermediateBase.h"
#include <array>
#include <stdlib.h>
#include <ctime>
#include <iostream>
...
#include <cmath>
using namespace std;
double GensolverInterBase::eval_gap(void) {
  int i;
  double gap;
  gap = 0;
  for (i = 0; i < 6; i++)
...
void GensolverInterBase::set_defaults(void) {
...
void GensolverInterBase::setup_pointers(void) {
  work.y = work.x + 4;
  work.s = work.x + 4;
  work.z = work.x + 10;
  vars.Pg = work.x + 0;
...
void GensolverInterBase::setup_indexing(void) {
  setup_pointers();
}
void GensolverInterBase::set_start(void) {
  int i;
  for (i = 0; i < 4; i++)
    work.x[i] = 0;
  for (i = 0; i < 0; i++)
...
double GensolverInterBase::eval_objv(void) {
  int i;
  double objv;
  ...
  objv += params.c0[0]+work.quad_399913824256[0]+work.quad_608032620544[0]+work.quad_643747205120[0]+work.quad_311816339456[0]+work.quad_155015135232[0]+work.quad_497818112000[0];
  return objv;
}
void GensolverInterBase::fillrhs_aff(void) {
  int i;
  double *r1, *r2, *r3, *r4;
  r1 = work.rhs;
  r2 = work.rhs + 4;
...
void GensolverInterBase::fillrhs_cc(void) {
  int i;
  double *r2;
...
void GensolverInterBase::refine(double *target, double *var) {
  int i, j;
...
double GensolverInterBase::calc_ineq_resid_squared(void) {
...
double GensolverInterBase::calc_eq_resid_squared(void) {
...
void GensolverInterBase::better_start(void) {
...
void GensolverInterBase::fillrhs_start(void) {
...
long GensolverInterBase::solve(void) {
  int i;
  int iter;
  double *dx, *ds, *dy, *dz;
  double minval;
...
void GensolverInterBase::tic(void) {
  tic_timestart = clock();
}
float GensolverInterBase::toc(void) {
...
float GensolverInterBase::tocq(void) {
  clock_t tic_timestop;
  tic_timestop = clock();
  return (float)(tic_timestop - tic_timestart) / CLOCKS_PER_SEC;
}
void GensolverInterBase::printmatrix(char *name, double *A, int m, int n, int sparse) {
  int i, j;
  printf("%s = [...\n", name);
  for (i = 0; i < m; i++) {
    for (j = 0; j < n; j++)
...
double GensolverInterBase::unif(double lower, double upper) {
...
#define EPS 1.2e-7
#define RNMX (1.0-EPS)
float GensolverInterBase::ran1(long*idum, int reset) {
  int j;
  long k;
  static long iy=0;
  static long iv[NTAB];
...
/* Next function is from numerical recipes in C. */
float GensolverInterBase::randn_internal(long *idum, int reset) {
  static int iset=0;
  static float gset;
  float fac, rsq, v1, v2;
  if (reset) {
...
double GensolverInterBase::randn(void) {
  return randn_internal(&global_seed, 0);
}
void GensolverInterBase::reset_rand(void) {
  srand(15);
  global_seed = 1;
...
void GensolverInterBase::multbymA(double *lhs, double *rhs) {
}
void GensolverInterBase::multbymAT(double *lhs, double *rhs) {
  lhs[0] = 0;
  lhs[1] = 0;
  lhs[2] = 0;
  lhs[3] = 0;
}
void GensolverInterBase::multbymG(double *lhs, double *rhs) {
  lhs[0] = -rhs[0]*(-1);
  lhs[1] = -rhs[0]*(1);
...
void GensolverInterBase::multbymGT(double *lhs, double *rhs) {
  lhs[0] = -rhs[0]*(-1)-rhs[1]*(1)-rhs[2]*(1)-rhs[3]*(-1)-rhs[4]*(-1)-rhs[5]*(1);
  lhs[1] = -rhs[2]*(-1)-rhs[3]*(1);
  lhs[2] = -rhs[4]*(1)-rhs[5]*(-1);
  lhs[3] = 0;
}
void GensolverInterBase::multbyP(double *lhs, double *rhs) {
...
void GensolverInterBase::fillq(void) {
  ...
  work.q[3] = 2*(-params.Vg_N_avg[0]-params.Thetag_N_avg[0]+params.vg_N[0])*work.frac_121674190848;
}
void GensolverInterBase::fillh(void) {
...
void GensolverInterBase::fillb(void) {
}
void GensolverInterBase::pre_ops(void) {
...
void GensolverInterBase::ldl_solve(double *target, double *var) {
  int i;
...
void GensolverInterBase::ldl_factor(void) {
  work.d[0] = work.KKT[0];
  if (work.d[0] < 0)
...
double GensolverInterBase::check_factorization(void) {
...
void GensolverInterBase::matrix_multiply(double *result, double *source) {
...
double GensolverInterBase::check_residual(double *target, double *multiplicand) {
...
void GensolverInterBase::fill_KKT(void) {
...
/* Description: Basic test harness for solver.c. */
GensolverInterBase::GensolverInterBase( int numberOfCases, double cost2, double cost1, double cost0, double PgenMax, double PgenMin, double RgenMax, double RgenMin, double Beta, double InnerBeta, double Gamma, double innerGamma ) {
...
GensolverInterBase::~GensolverInterBase() {
}
#define NUMTESTS 0
void GensolverInterBase::mainsolve(int outerAPPIt, int APPItCount, double gsRho, double Pgenprev, double Pgenavg, double Powerprice, double Angpriceavg, double Angavg, double Angprice, double PgenAPP, double PgenAPPInner, double PgenNextAPP, double PgenPrevAPP, double AAPP, double BAPP, double DAPP, double LambAPP1, double LambAPP2, double LambAPP3, double LambAPP4, double BSC[], double lambdaSC[]) {
  int num_iters;
#if (NUMTESTS > 0)
  int i;
  double time;
  ...
  set_defaults();
  setup_indexing();
  load_default_data(outerAPPIt, APPItCount, gsRho, Pgenprev, Pgenavg, Powerprice, Angpriceavg, Angavg, Angprice, PgenAPP, PgenAPPInner, PgenNextAPP, PgenPrevAPP, AAPP, BAPP, DAPP, LambAPP1, LambAPP2, LambAPP3, LambAPP4, BSC, lambdaSC);
...
void GensolverInterBase::load_default_data(int outerAPPIt, int APPItCount, double gsRho, double Pgenprev, double Pgenavg, double Powerprice, double Angpriceavg, double Angavg, double Angprice, double PgenAPP, double PgenAPPInner, double PgenNextAPP, double PgenPrevAPP, double AAPP, double BAPP, double DAPP, double LambAPP1, double LambAPP2, double LambAPP3, double LambAPP4, double BSC[], double lambdaSC[]) {
  if (outerAPPIt==1) {
   if (APPItCount==1) {
    params.rho[0] = gsRho;
    params.PgNu[0] = PgenAPP;//0.0
    params.PgNextNu[0] = PgenNextAPP;//0.0
    params.PgPrevNu[0] = PgenPrevAPP;//0.0
    params.PgNuInner[0] = PgenAPPInner;
    params.A[0] = AAPP;//0.0
    params.B[0] = BAPP;//0.0
    params.D[0] = DAPP;//0.0
    params.lambda_1[0] = LambAPP1;//0.0
    params.lambda_2[0] = LambAPP2;//0.0
    params.lambda_3[0] = LambAPP3;//0.0
    params.lambda_4[0] = LambAPP4;//0.0
    for (int i=0; i<params.caseNumber; ++i) {
     params.BSC[i] = BSC[i];//0.0
     params.lambda_1SC[i] = lambdaSC[i];//0.0
    }
   }
   else {
    params.rho[0] = gsRho;
...
double GensolverInterBase::getPSol(void) {
 Piterate = (vars.Pg);
 return *Piterate;
}

double GensolverInterBase::getPNextSol(void) {
 PgNextiterate = (vars.PgNext);
 return *PgNextiterate;
}

double GensolverInterBase::getPPrevSol(){ 
 PgPreviterate = (vars.PgPrev);
 return *PgPreviterate;
}

double GensolverInterBase::getObj(void) {
 return (params.c2[0])*(*Piterate)*(*Piterate)+(params.c1[0])*(*Piterate)+(params.c0[0]);
}

double *GensolverInterBase::getThetaPtr(void){
Thiterate = (vars.Thetag);
return Thiterate;
}

double GensolverInterBase::getRMax() // start of getPMax function
{
	return params.RgMax[0];
} // end of getMax

double GensolverInterBase::getRMin() // start of getPMin function
{
	return params.RgMin[0];
} // end of getPMin

double GensolverInterBase::getPMax() // start of getPMax function
{
	return params.PgMax[0];
} // end of getMax

double GensolverInterBase::getPMin() // start of getPMin function
{
	return params.PgMin[0];
} // end of getPMin

double GensolverInterBase::getQuadCoeff() // start of getQuadCoeff function
{
	return params.c2[0]; 
} // end of getQuadCoeff

double GensolverInterBase::getLinCoeff() // start of getLinCoeff function
{
	return params.c1[0]; 
} // end of getLinCoeff

double GensolverInterBase::getConstCoeff() // start of getConstCoeff function
{
	return params.c0[0]; 
} // end of getConstCoeff

double GensolverInterBase::getBeta(){return params.beta[0];}
double GensolverInterBase::getGamma(){return params.gamma[0];}
double GensolverInterBase::getIntGamma(){return params.gammaSC[0];}
