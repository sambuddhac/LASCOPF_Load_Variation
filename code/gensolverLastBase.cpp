// solver.cpp
/* Produced by CVXGEN, 2019-02-05 18:20:07 -0500.  */
...
#include <cmath>
using namespace std;
double GensolverLastBase::eval_gap(void) {
  int i;
  double gap;
  gap = 0;
  for (i = 0; i < 6; i++)
...
void GensolverLastBase::set_defaults(void) {
...
void GensolverLastBase::setup_pointers(void) {
  work.y = work.x + 3;
  work.s = work.x + 3;
  work.z = work.x + 9;
  vars.Pg = work.x + 0;
...
void GensolverLastBase::setup_indexing(void) {
  setup_pointers();
}
void GensolverLastBase::set_start(void) {
  int i;
  for (i = 0; i < 3; i++)
    work.x[i] = 0;
  for (i = 0; i < 0; i++)
...
double GensolverLastBase::eval_objv(void) {
  int i;
  double objv;
  ...
  objv += params.c0[0]+work.quad_399913824256[0]+work.quad_608032620544[0]+work.quad_311816339456[0]+work.quad_155015135232[0]+work.quad_497818112000[0];
  return objv;
}
void GensolverLastBase::fillrhs_aff(void) {
  int i;
  double *r1, *r2, *r3, *r4;
  r1 = work.rhs;
  r2 = work.rhs + 3;
...
void GensolverLastBase::fillrhs_cc(void) {
  int i;
  double *r2;
...
void GensolverLastBase::refine(double *target, double *var) {
  int i, j;
...
double GensolverLastBase::calc_ineq_resid_squared(void) {
...
double GensolverLastBase::calc_eq_resid_squared(void) {
...
void GensolverLastBase::better_start(void) {
...
void GensolverLastBase::fillrhs_start(void) {
...
long GensolverLastBase::solve(void) {
  int i;
  int iter;
  double *dx, *ds, *dy, *dz;
  double minval;
...
//util.cpp
/* Produced by CVXGEN, 2019-02-05 18:20:08 -0500.  */
/* CVXGEN is Copyright (C) 2006-2017 Jacob Mattingley, jem@cvxgen.com. */
/* The code in this file is Copyright (C) 2006-2017 Jacob Mattingley. */
/* CVXGEN, or solvers produced by CVXGEN, cannot be used for commercial */
/* applications without prior written permission from Jacob Mattingley. */
...
void GensolverLastBase::tic(void) {
  tic_timestart = clock();
}
float GensolverLastBase::toc(void) {
...
float GensolverLastBase::tocq(void) {
  clock_t tic_timestop;
  tic_timestop = clock();
  return (float)(tic_timestop - tic_timestart) / CLOCKS_PER_SEC;
}
void GensolverLastBase::printmatrix(char *name, double *A, int m, int n, int sparse) {
  int i, j;
  printf("%s = [...\n", name);
  for (i = 0; i < m; i++) {
    for (j = 0; j < n; j++)
...
double GensolverLastBase::unif(double lower, double upper) {
...
#define EPS 1.2e-7
#define RNMX (1.0-EPS)
float GensolverLastBase::ran1(long*idum, int reset) {
  int j;
  long k;
  static long iy=0;
  static long iv[NTAB];
...
/* Next function is from numerical recipes in C. */
float GensolverLastBase::randn_internal(long *idum, int reset) {
  static int iset=0;
  static float gset;
  float fac, rsq, v1, v2;
  if (reset) {
...
double GensolverLastBase::randn(void) {
  return randn_internal(&global_seed, 0);
}
void GensolverLastBase::reset_rand(void) {
  srand(15);
  global_seed = 1;
...
void GensolverLastBase::multbymA(double *lhs, double *rhs) {
}
void GensolverLastBase::multbymAT(double *lhs, double *rhs) {
  lhs[0] = 0;
  lhs[1] = 0;
  lhs[2] = 0;
}
void GensolverLastBase::multbymG(double *lhs, double *rhs) {
  lhs[0] = -rhs[0]*(-1);
  lhs[1] = -rhs[0]*(1);
...
void GensolverLastBase::fillq(void) {
...
void GensolverLastBase::fillh(void) {
...
void GensolverLastBase::fillb(void) {
}
void GensolverLastBase::pre_ops(void) {
...
void GensolverLastBase::ldl_solve(double *target, double *var) {
  int i;
...
void GensolverLastBase::ldl_factor(void) {
  work.d[0] = work.KKT[0];
  if (work.d[0] < 0)
...
double GensolverLastBase::check_factorization(void) {
...
void GensolverLastBase::matrix_multiply(double *result, double *source) {
...
double GensolverLastBase::check_residual(double *target, double *multiplicand) {
...
void GensolverLastBase::fill_KKT(void) {
...
/* Description: Basic test harness for solver.c. */
GensolverLastBase::GensolverLastBase( int numberOfCases, double cost2, double cost1, double cost0, double PgenMax, double PgenMin, double RgenMax, double RgenMin, double Beta, double InnerBeta, double Gamma, double innerGamma, int nextPChoice ) {
...
GensolverLastBase::~GensolverLastBase() {
}
#define NUMTESTS 0
void GensolverLastBase::mainsolve(int outerAPPIt, int APPItCount, double gsRho, double Pgenprev, double Pgenavg, double Powerprice, double Angpriceavg, double Angavg, double Angprice, double PgenAPP, double PgenAPPInner, double PgenPrevAPP, double AAPP, double BAPP, double LambAPP3, double LambAPP4, double BSC[], double lambdaSC[]) {
  int num_iters;
#if (NUMTESTS > 0)
  int i;
  double time;
  ...
  set_defaults();
  setup_indexing();
  load_default_data(outerAPPIt, APPItCount, gsRho, Pgenprev, Pgenavg, Powerprice, Angpriceavg, Angavg, Angprice, PgenAPP, PgenAPPInner, PgenPrevAPP, AAPP, BAPP, LambAPP3, LambAPP4, BSC, lambdaSC);
...
void GensolverLastBase::load_default_data(int outerAPPIt, int APPItCount, double gsRho, double Pgenprev, double Pgenavg, double Powerprice, double Angpriceavg, double Angavg, double Angprice, double PgenAPP, double PgenAPPInner, double PgenPrevAPP, double AAPP, double BAPP, double LambAPP3, double LambAPP4, double BSC[], double lambdaSC[]) {
  if (outerAPPIt==1) {
   if (APPItCount==1) {
    params.rho[0] = gsRho;
    params.PgNu[0] = PgenAPP;//0.0
    params.PgPrevNu[0] = PgenPrevAPP;//0.0
    params.PgNext[0] = PgenAPP;//0.0
...
double GensolverLastBase::getPSol(void) {
 Piterate = (vars.Pg);
 return *Piterate;
}

double GensolverLastBase::getPNextSol(void) {
 return params.PgNext[0];
}

double GensolverLastBase::getPPrevSol(){ 
 PgPreviterate = (vars.PgPrev);
 return *PgPreviterate;
}

double GensolverLastBase::getObj(void) {
 return (params.c2[0])*(*Piterate)*(*Piterate)+(params.c1[0])*(*Piterate)+(params.c0[0]);
}

double *GensolverLastBase::getThetaPtr(void){
Thiterate = (vars.Thetag);
return Thiterate;
}

double GensolverLastBase::getRMax() // start of getPMax function
{
	return params.RgMax[0];
} // end of getMax

double GensolverLastBase::getRMin() // start of getPMin function
{
	return params.RgMin[0];
} // end of getPMin

double GensolverLastBase::getPMax() // start of getPMax function
{
	return params.PgMax[0];
} // end of getMax

double GensolverLastBase::getPMin() // start of getPMin function
{
	return params.PgMin[0];
} // end of getPMin

double GensolverLastBase::getQuadCoeff() // start of getQuadCoeff function
{
	return params.c2[0]; 
} // end of getQuadCoeff

double GensolverLastBase::getLinCoeff() // start of getLinCoeff function
{
	return params.c1[0]; 
} // end of getLinCoeff

double GensolverLastBase::getConstCoeff() // start of getConstCoeff function
{
	return params.c0[0]; 
} // end of getConstCoeff

double GensolverLastBase::getBeta(){return params.beta[0];}
double GensolverLastBase::getGamma(){return params.gamma[0];}
double GensolverLastBase::getIntGamma(){return params.gammaSC[0];}
